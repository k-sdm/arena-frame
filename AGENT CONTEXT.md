# Arena Frame - Agent Context

> This file provides context for AI coding agents working on this project.
> It describes the architecture, conventions, and key technical details
> needed to make safe, informed changes.

## Overview

E-ink photo frame displaying content from Are.na channels. Runs on Raspberry Pi Zero 2 W with a Pimoroni Inky Impression display (7.3" or 13.3").
...
Each module runs as an independent systemd service under `/etc/systemd/system/`:

| Service | Command |
|---|---|
| `arena-frame.service` | `python main.py` |
| `arena-buttons.service` | `python -m hardware.buttons` |
| `arena-led.service` | `python -m hardware.led` |
| `wifi-manager.service` | `python -m wifi.manager` |
| `wifi-portal-web.service` | `python -m portal.app` |
...
These decisions are already implemented. Understand them before making changes in related areas.

1. **Modular dithering** — Plugin-style algorithms in `display/dither.py`. New algorithms should implement the same interface.
2. **Content source abstraction** — `ContentSource` ABC in `sources/__init__.py`; `ArenaSource` is the primary implementation. New sources (RSS, local files, etc.) should subclass `ContentSource`.
3. **Display abstraction** — `EinkDisplay` wrapper in `display/eink.py` encapsulates Inky hardware access.
4. **Unified config** — Single `config.py` owns all path constants, config/state/error helpers. Do not scatter path definitions across modules.
5. **Python wifi-manager** — `wifi/manager.py` is a Python state machine that replaced an earlier bash script (`system/wifi-manager.sh`, kept for reference).
6. **Separate templates** — HTML lives in `portal/templates/`, not inline in Python.
7. **Direct imports** — `main.py` calls modules directly rather than spawning subprocesses.
...
- Canvas rotated 90° CCW before sending to display
...
- The 13.3" display uses GPIO16 for SPI chip select
- Buttons must avoid GPIO16 — currently only GPIO5 (Button A) is used

### WiFi Flow

1. Boot → wifi-manager checks for saved networks
2. No saved networks → Start AP mode ("ArenaFrame-Setup")
3. User connects to AP → Captive portal served at 192.168.4.1
4. User saves config → Switch to client mode
5. Connection fails → Return to AP mode

### Error Handling

- `/tmp/arena-frame-error` — JSON file with error type
- wifi-manager monitors for channel errors and restarts AP when needed
...
These are natural extension points for contributors:

- **`sources/local.py`** — Local folder content source (stub exists)
- **`display/dither.py`** — Custom dithering algorithms (Atkinson, etc.)
- **New content sources** — RSS, URL, or other APIs via `ContentSource` subclass
- **Display drivers** — Abstraction for non-Inky displays
...
- Captive portal auto-open is inconsistent on iOS
